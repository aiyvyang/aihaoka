...
const posts = computed(() => {
  if (selectCategory.value) {
    ...
    return themeposts.filter(
      (article: any) => article?.frontmatter?.tags && article?.frontmatter?.tags.includes(selectTag.value)
    )
  } else if (selectYear.value && selectMonth.value) {
    return themeposts.filter(
      (article: any) =>
        article?.frontmatter?.date &&
        new Date(article?.frontmatter?.date).getFullYear() + '' == selectYear.value &&
        new Date(article?.frontmatter?.date).getMonth() + 1 + '月' == selectMonth.value
    )
