<template>
  <!-- 文章内页底下的文章版权声明。 -->
  <section style="letter-spacing: 0.544px; text-align: center; opacity: 0.5; font-size: 12px; margin-bottom: 20px">
    <span>- END -</span>
  </section>
  <div class="articlecc">
    ...
    <ul class="ul">
      <li class="li">
        ...
        ><span class="span"
          ><a class="a" target="_blank" :href="theme?.article?.cc?.authorLink">{{
            frontmatter?.author ? frontmatter?.author : theme?.article?.cc?.author
          }}</a></span
        >
      </li>
      <li class="li">
        <label class="label">标题</label><span class="span">{{ frontmatter?.title }}</span>
      </li>
      <li class="li">
        <label class="label">时间</label><span class="span">{{ formatTime(frontmatter?.date) }}</span>
      </li>
      <li class="li link" @click="handleCopy(articleLink)">
        <label class="label">链接</label><span v-tooltip="'点击复制'" class="span">{{ articleLink }}</span>
      </li>
      <li class="li" v-if="theme?.article?.cc?.licenseLink">
        <label class="span label">版权</label
        ><span class="span"
          ><a class="a" target="_blank" :href="theme?.article?.cc?.licenseLink">{{
            theme?.article?.cc?.license
          }}</a></span
        >
      </li>
    </ul>
